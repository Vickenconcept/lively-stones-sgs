...
<div class="max-w-lg mx-auto p-6">
    ...
    <form action="{{ route('session_years.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="date" name="start_date" id="start_date" class="w-full border-gray-300 rounded shadow-sm" value="{{ old('start_date') }}" required>
            @error('start_date')
                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="date" name="end_date" id="end_date" class="w-full border-gray-300 rounded shadow-sm" value="{{ old('end_date') }}" required>
            @error('end_date')
                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
